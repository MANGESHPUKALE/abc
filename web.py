...
by Group No.03 (Atharva college Students)
""")
...
if buffer is None:
    ...
    if preds >= 0.5:
        out = ('After X-Ray tests, it has been  {:.2%} confirmed that the patient have tuberculosis'.format(
            preds[0][0]))

    else:
        out = ('After X-Ray tests, it has been  {:.2%} confirmed that the patient does not have tuberculosis'.format(
            1-preds[0][0]))
